...
from GalaxiaMusic.utils.database import (delete_playlist, get_playlist,
                                       get_playlist_names,
                                       save_playlist)
from GalaxiaMusic.utils.decorators.language import language, languageCB
...
async def check_playlist(client, message: Message, _):
    ...
    msg = _["playlist_4"]
    count = 0
    for edward in _playlist:
        _note = await get_playlist(message.from_user.id, edward)
        title = _note["title"]
        title = title.title()
