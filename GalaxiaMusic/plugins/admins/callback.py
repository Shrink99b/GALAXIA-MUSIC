...
from GalaxiaMusic import YouTube, app
from GalaxiaMusic.core.call import Galaxia
from GalaxiaMusic.misc import SUDOERS, db
from GalaxiaMusic.utils.database import (is_active_chat,
                                       is_music_playing, is_muted,
                                       is_nonadmin_chat, music_off,
                                       music_on, mute_off, mute_on,
                                       set_loop)
from GalaxiaMusic.utils.decorators.language import languageCB
from GalaxiaMusic.utils.formatters import seconds_to_min
from GalaxiaMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from GalaxiaMusic.utils.stream.autoclear import auto_clean
from GalaxiaMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Galaxia.pause_stream(chat_id)
        ...
        await Galaxia.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention)
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await Galaxia.stop_stream(chat_id)
        ...
        await Galaxia.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Galaxia.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Galaxia.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Galaxia.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Galaxia.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
